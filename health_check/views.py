...
class HealthCheckListAPIView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        for endpoint in Endpoint.objects.filter(is_active=True):
            latest = HealthCheckResult.objects.filter(
                endpoint=endpoint).order_by('-checked_at').first()
            if latest:
    ...
    def list(self, request, *args, **kwargs):
        ...
        up_count = sum(1 for endpoint in endpoints if HealthCheckResult.objects.filter(
            endpoint=endpoint, is_up=True).order_by('-checked_at').first())
